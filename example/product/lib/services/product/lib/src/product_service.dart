import 'product_orm.dart';
import '../proto_generated/product.pb.dart';
import '../proto_generated/product.pbgrpc.dart';
...
class ProductService extends ProductServiceBase {
  ...
  ProductService(this.pgConnection) : super();
...
Future<GetResponse> tryToMaterialize(
  ...
  var productOrm = Product_ORM(newConnection(connection));

  return (await productOrm.materializeFromProto3Json(proto3Json)).fold((l) {
    return GetResponse()
      ..result = false
    ...
    return GetResponse()
      ..result = true
      ..mergeFromProto3Json(productOrm.recordToProto3Json());
  });
}

Future<InsertResponse> executeInsert(List<ProductToInsert> data,
    ...
    if (inTransaction) {
      ...
      await pgconnection.transaction((connection) async {
        response.errorMessage.addAll(await performAllInserts(
            data, Product_ORM.fromConnection(connection), inTransaction));
      });
    } else {
      response.errorMessage.addAll(await performAllInserts(
          data, Product_ORM(newConnection(pgconnection)), inTransaction));
    }
...
Future<List<String>> performAllInserts(
    List<ProductToInsert> data, Product_ORM orm, bool inTransaction) async {
  var result = <String>[];
  for (var product in data) {
    var error = (await tryToInsert(
        orm, product.toProto3Json() as Map<String, dynamic>));
    if (error.isNotEmpty) {
      result.addAll(error);
      if (inTransaction) {
        break;
...
Future<List<String>> tryToInsert(
    Product_ORM orm, Map<String, dynamic> data) async {
  try {
    var result = <String>[];
...
Future<UpdateResponse> executeUpdate(List<Product> data, bool inTransaction,
...
Future<List<String>> performAllUpdates(
    List<Product> data, Product_ORM orm, bool inTransaction) async {
  var result = <String>[];
...
Future<DeleteResponse> executeDelete(List<ProductPk> pkData, bool inTransaction,
    PostgreSQLConnection connection) async {
    ...
    if (inTransaction) {
      ...
      await pgconnection.transaction((connection) async {
        response.errorMessage.addAll(await performAllDeletes(
            pkData, Product_ORM.fromConnection(connection), inTransaction));
      });
    } else {
      response.errorMessage.addAll(await performAllDeletes(
          pkData, Product_ORM(newConnection(pgconnection)), inTransaction));
    }
...
Future<List<String>> performAllDeletes(
    List<ProductPk> pkData, Product_ORM orm, bool inTransaction) async {
  var result = <String>[];
...
Future<SelectResponse> tryToSelect(
  ...
  var ds = ((await Product_ORM(connection).select(
