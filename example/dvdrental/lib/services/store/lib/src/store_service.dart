import 'store_orm.dart';
import '../proto_generated/store.pb.dart';
import '../proto_generated/store.pbgrpc.dart';
...
class StoreService extends StoreServiceBase {
  final PostgreSQLConnection pgConnection;
  StoreService(this.pgConnection) : super();
...
void resolveColumns(Columns columns, SelectableColumns column) {
  switch (column) {
    case SelectableColumns.storeId:
      columns.store_id;
      break;
    case SelectableColumns.managerStaffId:
...
Future<GetResponse> tryToMaterialize(
  ...
  var storeOrm = Store_ORM(newConnection(connection));

  return (await storeOrm.materializeFromProto3Json(proto3Json)).fold((l) {
    return GetResponse()
      ..result = false
    ...
    return GetResponse()
      ..result = true
      ..mergeFromProto3Json(storeOrm.recordToProto3Json());
  });
}

Future<InsertResponse> executeInsert(List<StoreToInsert> data,
    ...
    if (inTransaction) {
      ...
      await pgconnection.transaction((connection) async {
        response.errorMessage.addAll(await performAllInserts(
            data, Store_ORM.fromConnection(connection), inTransaction));
      });
    } else {
      response.errorMessage.addAll(await performAllInserts(
          data, Store_ORM(newConnection(pgconnection)), inTransaction));
    }
...
Future<List<String>> performAllInserts(
    List<StoreToInsert> data, Store_ORM orm, bool inTransaction) async {
  var result = <String>[];
  for (var store in data) {
    var error =
        (await tryToInsert(orm, store.toProto3Json() as Map<String, dynamic>));
    if (error.isNotEmpty) {
      result.addAll(error);
      if (inTransaction) {
        break;
...
Future<List<String>> tryToInsert(
    Store_ORM orm, Map<String, dynamic> data) async {
  try {
    var result = <String>[];
...
Future<UpdateResponse> executeUpdate(List<Store> data, bool inTransaction,
...
Future<List<String>> performAllUpdates(
    List<Store> data, Store_ORM orm, bool inTransaction) async {
  var result = <String>[];
...
Future<DeleteResponse> executeDelete(List<StorePk> pkData, bool inTransaction,
    PostgreSQLConnection connection) async {
    ...
    if (inTransaction) {
      ...
      await pgconnection.transaction((connection) async {
        response.errorMessage.addAll(await performAllDeletes(
            pkData, Store_ORM.fromConnection(connection), inTransaction));
      });
    } else {
      response.errorMessage.addAll(await performAllDeletes(
          pkData, Store_ORM(newConnection(pgconnection)), inTransaction));
    }
...
Future<List<String>> performAllDeletes(
    List<StorePk> pkData, Store_ORM orm, bool inTransaction) async {
  var result = <String>[];
...
Future<SelectResponse> tryToSelect(
  ...
  var ds = ((await Store_ORM(connection).select(
