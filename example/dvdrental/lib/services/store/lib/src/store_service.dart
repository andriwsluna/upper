...
Future<List<String>> performAllInserts(
  ...
  for (var store in data) {
    var error = (await tryToInsert(
        orm, store.toProto3Json() as Map<String, dynamic>));
    if (error.isNotEmpty) {
      result.addAll(error);
      if (inTransaction) {
        break;
...
Future<UpdateResponse> executeUpdate(List<Store> data,
    bool inTransaction, PostgreSQLConnection connection) async {
...
Future<DeleteResponse> executeDelete(List<StorePk> pkData,
    bool inTransaction, PostgreSQLConnection connection) async {
