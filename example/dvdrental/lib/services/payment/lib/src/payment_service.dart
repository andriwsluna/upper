import 'payment_orm.dart';
import '../proto_generated/payment.pb.dart';
import '../proto_generated/payment.pbgrpc.dart';
...
class PaymentService extends PaymentServiceBase {
  ...
  PaymentService(this.pgConnection) : super();
...
void resolveColumns(Columns columns, SelectableColumns column) {
  switch (column) {
    case SelectableColumns.paymentId:
      columns.payment_id;
      break;
    ...
    case SelectableColumns.staffId:
      columns.staff_id;
      break;
    case SelectableColumns.rentalId:
      columns.rental_id;
      break;
    case SelectableColumns.amount:
      columns.amount;
      break;
...
Future<GetResponse> tryToMaterialize(
  ...
  var paymentOrm = Payment_ORM(newConnection(connection));

  return (await paymentOrm.materializeFromProto3Json(proto3Json)).fold((l) {
    return GetResponse()
      ..result = false
    ...
    return GetResponse()
      ..result = true
      ..mergeFromProto3Json(paymentOrm.recordToProto3Json());
  });
}

Future<InsertResponse> executeInsert(List<PaymentToInsert> data,
    ...
    if (inTransaction) {
      ...
      await pgconnection.transaction((connection) async {
        response.errorMessage.addAll(await performAllInserts(
            data, Payment_ORM.fromConnection(connection), inTransaction));
      });
    } else {
      response.errorMessage.addAll(await performAllInserts(
          data, Payment_ORM(newConnection(pgconnection)), inTransaction));
    }
...
Future<List<String>> performAllInserts(
    List<PaymentToInsert> data, Payment_ORM orm, bool inTransaction) async {
  var result = <String>[];
  for (var payment in data) {
    var error = (await tryToInsert(
        orm, payment.toProto3Json() as Map<String, dynamic>));
    if (error.isNotEmpty) {
      result.addAll(error);
      if (inTransaction) {
        break;
...
Future<List<String>> tryToInsert(
    Payment_ORM orm, Map<String, dynamic> data) async {
  try {
    var result = <String>[];
...
Future<List<String>> performAllUpdates(
    List<Payment> data, Payment_ORM orm, bool inTransaction) async {
  var result = <String>[];
...
Future<DeleteResponse> executeDelete(List<PaymentPk> pkData,
  ...
  try {
    if (inTransaction) {
      ...
      response.errorMessage.addAll(await performAllDeletes(
          pkData, Payment_ORM(newConnection(pgconnection)), inTransaction));
    }
...
Future<List<String>> performAllDeletes(
    List<PaymentPk> pkData, Payment_ORM orm, bool inTransaction) async {
  var result = <String>[];
...
Future<SelectResponse> tryToSelect(
  ...
  var ds = ((await Payment_ORM(connection).select(
