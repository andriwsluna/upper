import 'language_orm.dart';
import '../proto_generated/language.pb.dart';
import '../proto_generated/language.pbgrpc.dart';
...
class LanguageService extends LanguageServiceBase {
  final PostgreSQLConnection pgConnection;
  LanguageService(this.pgConnection) : super();
...
Future<GetResponse> tryToMaterialize(
  ...
  var languageOrm = Language_ORM(newConnection(connection));

  return (await languageOrm.materializeFromProto3Json(proto3Json)).fold((l) {
    return GetResponse()
      ..result = false
    ...
    return GetResponse()
      ..result = true
      ..mergeFromProto3Json(languageOrm.recordToProto3Json());
  });
}

Future<InsertResponse> executeInsert(List<LanguageToInsert> data,
    ...
    if (inTransaction) {
      ...
      await pgconnection.transaction((connection) async {
        response.errorMessage.addAll(await performAllInserts(
            data, Language_ORM.fromConnection(connection), inTransaction));
      });
    } else {
      response.errorMessage.addAll(await performAllInserts(
          data, Language_ORM(newConnection(pgconnection)), inTransaction));
    }
...
Future<List<String>> performAllInserts(
    List<LanguageToInsert> data, Language_ORM orm, bool inTransaction) async {
  var result = <String>[];
  for (var language in data) {
    var error = (await tryToInsert(
        orm, language.toProto3Json() as Map<String, dynamic>));
    if (error.isNotEmpty) {
      result.addAll(error);
      if (inTransaction) {
        break;
...
Future<List<String>> tryToInsert(
    Language_ORM orm, Map<String, dynamic> data) async {
  try {
    var result = <String>[];
...
Future<UpdateResponse> executeUpdate(List<Language> data, bool inTransaction,
...
Future<List<String>> performAllUpdates(
    List<Language> data, Language_ORM orm, bool inTransaction) async {
  var result = <String>[];
...
Future<DeleteResponse> executeDelete(List<LanguagePk> pkData,
    bool inTransaction, PostgreSQLConnection connection) async {
    ...
    if (inTransaction) {
      ...
      await pgconnection.transaction((connection) async {
        response.errorMessage.addAll(await performAllDeletes(
            pkData, Language_ORM.fromConnection(connection), inTransaction));
      });
    } else {
      response.errorMessage.addAll(await performAllDeletes(
          pkData, Language_ORM(newConnection(pgconnection)), inTransaction));
    }
...
Future<List<String>> performAllDeletes(
    List<LanguagePk> pkData, Language_ORM orm, bool inTransaction) async {
  var result = <String>[];
...
Future<SelectResponse> tryToSelect(
  ...
  var ds = ((await Language_ORM(connection).select(
