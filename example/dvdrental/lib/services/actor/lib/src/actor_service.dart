...
Future<List<String>> performAllInserts(
  ...
  for (var actor in data) {
    var error = (await tryToInsert(
        orm, actor.toProto3Json() as Map<String, dynamic>));
    if (error.isNotEmpty) {
      result.addAll(error);
      if (inTransaction) {
        break;
...
Future<UpdateResponse> executeUpdate(List<Actor> data,
    bool inTransaction, PostgreSQLConnection connection) async {
...
Future<DeleteResponse> executeDelete(List<ActorPk> pkData,
    bool inTransaction, PostgreSQLConnection connection) async {
