import 'actor_orm.dart';
import '../proto_generated/actor.pb.dart';
import '../proto_generated/actor.pbgrpc.dart';
...
class ActorService extends ActorServiceBase {
  final PostgreSQLConnection pgConnection;
  ActorService(this.pgConnection) : super();
...
Future<GetResponse> tryToMaterialize(
  ...
  var actorOrm = Actor_ORM(newConnection(connection));

  return (await actorOrm.materializeFromProto3Json(proto3Json)).fold((l) {
    return GetResponse()
      ..result = false
    ...
    return GetResponse()
      ..result = true
      ..mergeFromProto3Json(actorOrm.recordToProto3Json());
  });
}

Future<InsertResponse> executeInsert(List<ActorToInsert> data,
    ...
    if (inTransaction) {
      ...
      await pgconnection.transaction((connection) async {
        response.errorMessage.addAll(await performAllInserts(
            data, Actor_ORM.fromConnection(connection), inTransaction));
      });
    } else {
      response.errorMessage.addAll(await performAllInserts(
          data, Actor_ORM(newConnection(pgconnection)), inTransaction));
    }
...
Future<List<String>> performAllInserts(
    List<ActorToInsert> data, Actor_ORM orm, bool inTransaction) async {
  var result = <String>[];
  for (var actor in data) {
    var error =
        (await tryToInsert(orm, actor.toProto3Json() as Map<String, dynamic>));
    if (error.isNotEmpty) {
      result.addAll(error);
      if (inTransaction) {
        break;
...
Future<List<String>> tryToInsert(
    Actor_ORM orm, Map<String, dynamic> data) async {
  try {
    var result = <String>[];
...
Future<UpdateResponse> executeUpdate(List<Actor> data, bool inTransaction,
...
Future<List<String>> performAllUpdates(
    List<Actor> data, Actor_ORM orm, bool inTransaction) async {
  var result = <String>[];
...
Future<DeleteResponse> executeDelete(List<ActorPk> pkData, bool inTransaction,
    PostgreSQLConnection connection) async {
    ...
    if (inTransaction) {
      ...
      await pgconnection.transaction((connection) async {
        response.errorMessage.addAll(await performAllDeletes(
            pkData, Actor_ORM.fromConnection(connection), inTransaction));
      });
    } else {
      response.errorMessage.addAll(await performAllDeletes(
          pkData, Actor_ORM(newConnection(pgconnection)), inTransaction));
    }
...
Future<List<String>> performAllDeletes(
    List<ActorPk> pkData, Actor_ORM orm, bool inTransaction) async {
  var result = <String>[];
...
Future<SelectResponse> tryToSelect(
  ...
  var ds = ((await Actor_ORM(connection).select(
