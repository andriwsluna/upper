import 'category_orm.dart';
import '../proto_generated/category.pb.dart';
import '../proto_generated/category.pbgrpc.dart';
...
class CategoryService extends CategoryServiceBase {
  ...
  CategoryService(this.pgConnection) : super();
...
void resolveColumns(Columns columns, SelectableColumns column) {
  switch (column) {
    case SelectableColumns.categoryId:
      columns.category_id;
      break;
...
Future<GetResponse> tryToMaterialize(
  ...
  var categoryOrm = Category_ORM(newConnection(connection));

  return (await categoryOrm.materializeFromProto3Json(proto3Json)).fold((l) {
    return GetResponse()
      ..result = false
    ...
    return GetResponse()
      ..result = true
      ..mergeFromProto3Json(categoryOrm.recordToProto3Json());
  });
}

Future<InsertResponse> executeInsert(List<CategoryToInsert> data,
    ...
    if (inTransaction) {
      ...
      await pgconnection.transaction((connection) async {
        response.errorMessage.addAll(await performAllInserts(
            data, Category_ORM.fromConnection(connection), inTransaction));
      });
    } else {
      response.errorMessage.addAll(await performAllInserts(
          data, Category_ORM(newConnection(pgconnection)), inTransaction));
    }
...
Future<List<String>> performAllInserts(
    List<CategoryToInsert> data, Category_ORM orm, bool inTransaction) async {
  var result = <String>[];
  for (var category in data) {
    var error = (await tryToInsert(
        orm, category.toProto3Json() as Map<String, dynamic>));
    if (error.isNotEmpty) {
      result.addAll(error);
      if (inTransaction) {
        break;
...
Future<List<String>> tryToInsert(
    Category_ORM orm, Map<String, dynamic> data) async {
  try {
    var result = <String>[];
...
Future<UpdateResponse> executeUpdate(List<Category> data, bool inTransaction,
...
Future<List<String>> performAllUpdates(
    List<Category> data, Category_ORM orm, bool inTransaction) async {
  var result = <String>[];
...
Future<DeleteResponse> executeDelete(List<CategoryPk> pkData,
    bool inTransaction, PostgreSQLConnection connection) async {
    ...
    if (inTransaction) {
      ...
      await pgconnection.transaction((connection) async {
        response.errorMessage.addAll(await performAllDeletes(
            pkData, Category_ORM.fromConnection(connection), inTransaction));
      });
    } else {
      response.errorMessage.addAll(await performAllDeletes(
          pkData, Category_ORM(newConnection(pgconnection)), inTransaction));
    }
...
Future<List<String>> performAllDeletes(
    List<CategoryPk> pkData, Category_ORM orm, bool inTransaction) async {
  var result = <String>[];
...
Future<SelectResponse> tryToSelect(
  ...
  var ds = ((await Category_ORM(connection).select(
