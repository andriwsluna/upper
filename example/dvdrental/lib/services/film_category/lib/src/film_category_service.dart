import 'film_category_orm.dart';
import '../proto_generated/film_category.pb.dart';
import '../proto_generated/film_category.pbgrpc.dart';
...
class Film_categoryService extends Film_categoryServiceBase {
  final PostgreSQLConnection pgConnection;
  Film_categoryService(this.pgConnection) : super();
...
Future<GetResponse> tryToMaterialize(
  ...
  return (await filmCategoryOrm.materializeFromProto3Json(proto3Json)).fold(
    ...
    return GetResponse()
      ..result = true
      ..mergeFromProto3Json(filmCategoryOrm.recordToProto3Json());
  });
}

Future<InsertResponse> executeInsert(List<Film_categoryToInsert> data,
    ...
    if (inTransaction) {
      ...
      await pgconnection.transaction((connection) async {
        response.errorMessage.addAll(await performAllInserts(
            data, Film_category_ORM.fromConnection(connection), inTransaction));
      });
    } else {
      response.errorMessage.addAll(await performAllInserts(
          data, Film_category_ORM(newConnection(pgconnection)), inTransaction));
    }
...
Future<List<String>> performAllInserts(List<Film_categoryToInsert> data,
  ...
  for (var film_category in data) {
    var error = (await tryToInsert(
        orm, film_category.toProto3Json() as Map<String, dynamic>));
    if (error.isNotEmpty) {
      result.addAll(error);
      if (inTransaction) {
        break;
...
Future<List<String>> tryToInsert(
    Film_category_ORM orm, Map<String, dynamic> data) async {
  try {
    var result = <String>[];
...
Future<UpdateResponse> executeUpdate(List<Film_category> data,
    bool inTransaction, PostgreSQLConnection connection) async {
...
Future<List<String>> performAllUpdates(
    List<Film_category> data, Film_category_ORM orm, bool inTransaction) async {
  var result = <String>[];
...
Future<DeleteResponse> executeDelete(List<Film_categoryPk> pkData,
    bool inTransaction, PostgreSQLConnection connection) async {
...
Future<SelectResponse> tryToSelect(
  ...
  var ds = ((await Film_category_ORM(connection).select(
