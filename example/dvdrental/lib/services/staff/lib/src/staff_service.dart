...
Future<List<String>> performAllInserts(
  ...
  for (var staff in data) {
    var error = (await tryToInsert(
        orm, staff.toProto3Json() as Map<String, dynamic>));
    if (error.isNotEmpty) {
      result.addAll(error);
      if (inTransaction) {
        break;
...
Future<UpdateResponse> executeUpdate(List<Staff> data,
    bool inTransaction, PostgreSQLConnection connection) async {
...
Future<DeleteResponse> executeDelete(List<StaffPk> pkData,
    bool inTransaction, PostgreSQLConnection connection) async {
