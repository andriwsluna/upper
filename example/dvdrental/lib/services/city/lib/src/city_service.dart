import 'city_orm.dart';
import '../proto_generated/city.pb.dart';
import '../proto_generated/city.pbgrpc.dart';
...
class CityService extends CityServiceBase {
  ...
  CityService(this.pgConnection) : super();
...
Future<GetResponse> tryToMaterialize(
  ...
  var cityOrm = City_ORM(newConnection(connection));

  return (await cityOrm.materializeFromProto3Json(proto3Json)).fold((l) {
    return GetResponse()
      ..result = false
    ...
    return GetResponse()
      ..result = true
      ..mergeFromProto3Json(cityOrm.recordToProto3Json());
  });
}

Future<InsertResponse> executeInsert(List<CityToInsert> data,
    ...
    if (inTransaction) {
      ...
      await pgconnection.transaction((connection) async {
        response.errorMessage.addAll(await performAllInserts(
            data, City_ORM.fromConnection(connection), inTransaction));
      });
    } else {
      response.errorMessage.addAll(await performAllInserts(
          data, City_ORM(newConnection(pgconnection)), inTransaction));
    }
...
Future<List<String>> tryToInsert(
    City_ORM orm, Map<String, dynamic> data) async {
  try {
    var result = <String>[];
...
Future<List<String>> performAllUpdates(
    List<City> data, City_ORM orm, bool inTransaction) async {
  var result = <String>[];
...
Future<DeleteResponse> executeDelete(List<CityPk> pkData, bool inTransaction,
  ...
  try {
    if (inTransaction) {
      ...
      response.errorMessage.addAll(await performAllDeletes(
          pkData, City_ORM(newConnection(pgconnection)), inTransaction));
    }
...
Future<List<String>> performAllDeletes(
    List<CityPk> pkData, City_ORM orm, bool inTransaction) async {
  var result = <String>[];
...
Future<SelectResponse> tryToSelect(
  ...
  var ds = ((await City_ORM(connection).select(
