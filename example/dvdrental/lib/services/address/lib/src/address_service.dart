import 'address_orm.dart';
import '../proto_generated/address.pb.dart';
import '../proto_generated/address.pbgrpc.dart';
...
class AddressService extends AddressServiceBase {
  ...
  AddressService(this.pgConnection) : super();
...
void resolveColumns(Columns columns, SelectableColumns column) {
  switch (column) {
    ...
    case SelectableColumns.address:
      columns.address;
      break;
    case SelectableColumns.address2:
      columns.address2;
      break;
    case SelectableColumns.district:
      columns.district;
      break;
    case SelectableColumns.cityId:
      columns.city_id;
      break;
    case SelectableColumns.postalCode:
      columns.postal_code;
      break;
    case SelectableColumns.phone:
      columns.phone;
      break;
...
Future<GetResponse> tryToMaterialize(
  ...
  var addressOrm = Address_ORM(newConnection(connection));

  return (await addressOrm.materializeFromProto3Json(proto3Json)).fold((l) {
    return GetResponse()
      ..result = false
    ...
    return GetResponse()
      ..result = true
      ..mergeFromProto3Json(addressOrm.recordToProto3Json());
  });
}

Future<InsertResponse> executeInsert(List<AddressToInsert> data,
    ...
    if (inTransaction) {
      ...
      await pgconnection.transaction((connection) async {
        response.errorMessage.addAll(await performAllInserts(
            data, Address_ORM.fromConnection(connection), inTransaction));
      });
    } else {
      response.errorMessage.addAll(await performAllInserts(
          data, Address_ORM(newConnection(pgconnection)), inTransaction));
    }
...
Future<List<String>> performAllInserts(
    List<AddressToInsert> data, Address_ORM orm, bool inTransaction) async {
  var result = <String>[];
  for (var address in data) {
    var error = (await tryToInsert(
        orm, address.toProto3Json() as Map<String, dynamic>));
    if (error.isNotEmpty) {
      result.addAll(error);
      if (inTransaction) {
        break;
...
Future<List<String>> tryToInsert(
    Address_ORM orm, Map<String, dynamic> data) async {
  try {
    var result = <String>[];
...
Future<UpdateResponse> executeUpdate(List<Address> data, bool inTransaction,
...
Future<List<String>> performAllUpdates(
    List<Address> data, Address_ORM orm, bool inTransaction) async {
  var result = <String>[];
...
Future<DeleteResponse> executeDelete(List<AddressPk> pkData, bool inTransaction,
    PostgreSQLConnection connection) async {
    ...
    if (inTransaction) {
      ...
      await pgconnection.transaction((connection) async {
        response.errorMessage.addAll(await performAllDeletes(
            pkData, Address_ORM.fromConnection(connection), inTransaction));
      });
    } else {
      response.errorMessage.addAll(await performAllDeletes(
          pkData, Address_ORM(newConnection(pgconnection)), inTransaction));
    }
...
Future<List<String>> performAllDeletes(
    List<AddressPk> pkData, Address_ORM orm, bool inTransaction) async {
  var result = <String>[];
...
Future<SelectResponse> tryToSelect(
  ...
  var ds = ((await Address_ORM(connection).select(
