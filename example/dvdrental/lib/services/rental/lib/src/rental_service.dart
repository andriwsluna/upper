import 'rental_orm.dart';
import '../proto_generated/rental.pb.dart';
import '../proto_generated/rental.pbgrpc.dart';
...
class RentalService extends RentalServiceBase {
  final PostgreSQLConnection pgConnection;
  RentalService(this.pgConnection) : super();
...
void resolveColumns(Columns columns, SelectableColumns column) {
  switch (column) {
    case SelectableColumns.rentalId:
      columns.rental_id;
      break;
    case SelectableColumns.rentalDate:
      columns.rental_date;
      break;
    ...
    case SelectableColumns.customerId:
      columns.customer_id;
      break;
    case SelectableColumns.returnDate:
      columns.return_date;
      break;
...
Future<GetResponse> tryToMaterialize(
  ...
  var rentalOrm = Rental_ORM(newConnection(connection));

  return (await rentalOrm.materializeFromProto3Json(proto3Json)).fold((l) {
    return GetResponse()
      ..result = false
    ...
    return GetResponse()
      ..result = true
      ..mergeFromProto3Json(rentalOrm.recordToProto3Json());
  });
}

Future<InsertResponse> executeInsert(List<RentalToInsert> data,
    ...
    if (inTransaction) {
      ...
      await pgconnection.transaction((connection) async {
        response.errorMessage.addAll(await performAllInserts(
            data, Rental_ORM.fromConnection(connection), inTransaction));
      });
    } else {
      response.errorMessage.addAll(await performAllInserts(
          data, Rental_ORM(newConnection(pgconnection)), inTransaction));
    }
...
Future<List<String>> tryToInsert(
    Rental_ORM orm, Map<String, dynamic> data) async {
  try {
    var result = <String>[];
...
Future<List<String>> performAllUpdates(
    List<Rental> data, Rental_ORM orm, bool inTransaction) async {
  var result = <String>[];
...
Future<DeleteResponse> executeDelete(List<RentalPk> pkData, bool inTransaction,
  ...
  try {
    if (inTransaction) {
      ...
      response.errorMessage.addAll(await performAllDeletes(
          pkData, Rental_ORM(newConnection(pgconnection)), inTransaction));
    }
...
Future<List<String>> performAllDeletes(
    List<RentalPk> pkData, Rental_ORM orm, bool inTransaction) async {
  var result = <String>[];
...
Future<SelectResponse> tryToSelect(
  ...
  var ds = ((await Rental_ORM(connection).select(
